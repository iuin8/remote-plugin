...
object SshConfigManager {
    private val logger = Logging.getLogger(SshConfigManager::class.java)
    
    private const val PLUGIN_INCLUDE_LINE = "Include ~/.ssh/gradle/remote-plugin/config"
    ...
    fun setupSshConfig(projectRootDir: File, projectName: String) {
        try {
            ...
            if (!projectSshConfig.exists()) {
                logger.debug("[remote-plugin] 项目未配置 SSH 配置文件，跳过 SSH 配置注入")
                return
            }
            
            logger.debug("[remote-plugin] 找到 SSH 配置文件：gradle/remote-plugin/.ssh/config.template")
    ...
    private fun ensureSystemConfigIncludesPlugin() {
        ...
        // 确保插件托管的配置文件存在，避免 SSH Include 报错
        val pluginConfigDir = File(userHome, ".ssh/gradle/remote-plugin")
        val managedConfig = File(pluginConfigDir, "config")
        ensureSshDirAndConfigExists(pluginConfigDir, managedConfig)

        val existingContent = systemConfig.readText()
        val normalizedPluginLine = PLUGIN_INCLUDE_LINE.trim()

        // 如果已经包含了具体的 Include 指令，直接返回
        if (existingContent.lines().any { it.trim() == normalizedPluginLine }) {
            logger.debug("[remote-plugin] 系统 SSH 配置已包含 Gradle Remote Plugin 引用")
            return
        }
        ...
        logger.debug("[remote-plugin] 已将 Gradle Remote Plugin 配置引用注入到 ~/.ssh/config")
    }
    ...
    private fun ensureManagedConfigIncludesProject(
        ...
        logger.debug("[remote-plugin] 已将项目 SSH 配置（解析版）添加到插件管理配置中")
    }
}
